...
- **Nextcloud Talk Signaling + NATS**: High-performance signaling backend for Talk with WebSocket forwarding, and a NATS message bus. Configuration via `.env` plus a `gnatsd.conf`. fileciteturn5file2
...
- Signaling: `8082` (HTTP WS), NATS client: `4222`, NATS monitor: `8222` fileciteturn5file0 fileciteturn5file1 fileciteturn5file2
...
See the service READMEs for details: Nextcloud, Collabora, and Signaling. fileciteturn5file0 fileciteturn5file1 fileciteturn5file2
...
docker compose up -d
```
Nextcloud becomes available on `http://<host>:8080`, Collabora on `http://<host>:9980`, and the signaling service on `http://<host>:8082`. Put them behind your reverse proxy. fileciteturn5file0 fileciteturn5file1 fileciteturn5file2
...
Update each `.env` to include trusted proxies/domains as noted in the service READMEs. fileciteturn5file0 fileciteturn5file2

---

## Integrations inside Nextcloud

- **Office integration**: in Nextcloud → Administration → Office, select “Use your own server” and enter your Collabora URL (the public HTTPS host you routed to `:9980`). fileciteturn5file1
- **Talk high‑performance backend**: in Nextcloud → Administration → Talk, set the Signaling server URL to your public signaling host (the one routed to `:8082`). Configure TURN/STUN for NAT traversal. fileciteturn5file2
...
```
Service‑specific README files include more commands and details. fileciteturn5file0 fileciteturn5file1 fileciteturn5file2

---

## Backups (minimum viable)

- **Nextcloud**: back up the DB (SQL dumps) and the app/data volume. fileciteturn5file0
- **Collabora**: back up the named volume storing configuration/logs. fileciteturn5file1
- **Signaling/NATS**: back up signaling env values and your `gnatsd.conf`. fileciteturn5file2
...
- Don’t commit `.env` files. Generate strong secrets and rotate periodically. fileciteturn5file0
- Terminate TLS at the reverse proxy; only 443 should be public. fileciteturn5file0
- Restrict direct access to Collabora `:9980`, Signaling `:8082`, and NATS `:4222/:8222` to internal networks. fileciteturn5file1 fileciteturn5file2
- For Talk Signaling, generate fresh random keys and consider enabling NATS auth. fileciteturn5file2
...
```
Each service README includes update guidance (Nextcloud also uses the web updater for core/app updates). fileciteturn5file0
